...
class AppCompatPopupWindow extends PopupWindow {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0180R.styleable.PopupWindow, defStyleAttr, defStyleRes);
        if (a.hasValue(C0180R.styleable.PopupWindow_overlapAnchor)) {
            setSupportOverlapAnchor(a.getBoolean(C0180R.styleable.PopupWindow_overlapAnchor, false));
        }
        setBackgroundDrawable(a.getDrawable(C0180R.styleable.PopupWindow_android_popupBackground));
        int sdk = VERSION.SDK_INT;
        if (defStyleRes != 0 && sdk < 11 && a.hasValue(C0180R.styleable.PopupWindow_android_popupAnimationStyle)) {
            setAnimationStyle(a.getResourceId(C0180R.styleable.PopupWindow_android_popupAnimationStyle, -1));
        }
        a.recycle();
