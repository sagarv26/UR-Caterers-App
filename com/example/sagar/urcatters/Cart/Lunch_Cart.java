...
public class Lunch_Cart extends AppCompatActivity implements RecyclerItemTouchHelperListener {
    ...
    final String titleName = "Lunch";

    class C02881 implements OnClickListener {

        class C02861 implements DialogInterface.OnClickListener {
            C02861() {
            }

            public void onClick(DialogInterface dialog, int id) {
                Toast.makeText(Lunch_Cart.this.getApplicationContext(), "Cancelled", 0).show();
                dialog.cancel();
            }
        }

        class C02872 implements DialogInterface.OnClickListener {
            C02872() {
            }

            public void onClick(DialogInterface dialog, int id) {
                Toast.makeText(Lunch_Cart.this.getApplicationContext(), "Confirmed", 0).show();
            }
        }
        ...
        public void onClick(View v) {
            Builder alertDialogBuilder = new Builder(Lunch_Cart.this.mContext);
            alertDialogBuilder.setCancelable(false);
            alertDialogBuilder.setTitle("Lunch Order Confirmation");
            alertDialogBuilder.setMessage("Please Confirm Your Lunch Order and Move to One Step Process");
            alertDialogBuilder.setCancelable(false).setPositiveButton("OK", new C02872()).setNegativeButton("Cancel", new C02861());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.single_textView.setText("Your Lunch Cart");
        this.single_confirm_button.setOnClickListener(new C02881());
        ...
        if (viewHolder instanceof MyViewHolder) {
            ...
            snackbar.setAction((CharSequence) "UNDO", new OnClickListener() {
                public void onClick(View view) {
                    if (Long.valueOf(Lunch_Cart.this.bf_database.create(name, shift_name)).longValue() == -1) {
                        Toast.makeText(Lunch_Cart.this.getApplicationContext(), name + " : Not Added to Cart", 0).show();
                    } else {
                        Toast.makeText(Lunch_Cart.this.getApplicationContext(), name + " : Added Back to Cart", 0).show();
                    }
                    Lunch_Cart.this.adapter.restoreItem(deletedItem, deletedIndex);
                }
            });
